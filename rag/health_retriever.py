"""
health_retriever.py
Loads the health FAISS index and retrieves top-k relevant chunks
for a given query. Used by health_agent at runtime.
"""
import os
import pickle
import faiss
...
def _load():
    ...
    if _model is None:
        print("[RAG-Health] Loading retriever model...")
        _model = SentenceTransformer(MODEL_NAME)
    if _index is None:
        if not os.path.exists(IDX_PATH):
            raise FileNotFoundError(
                "Health FAISS index not found. "
                "Run: python rag/health_embedder.py"
            )
...
def retrieve_as_context(query: str, top_k: int = 3) -> str:
    ...
    context = "\n\nRELEVANT HEALTH KNOWLEDGE BASE CONTEXT:\n"
    context += "=" * 40 + "\n"
